...
export default function ExperiencePage() {
  return (
    <main className="page-shell">
      ...
      <section className="section experience-section"><SectionHeading eyebrow="Career timeline" title="Roles, responsibilities, and results." /><div className="experience-list">{timeline.map((item) => <details className="experience-entry" key={item.dates+item.role}><summary><span className="org-code" aria-hidden="true">{item.code}</span><div><span className="date">{item.dates}</span><h3>{item.role}</h3><p>{item.organization}</p>{item.keyResult && <CareerHighlight>{item.keyResult}</CareerHighlight>}</div><b aria-hidden="true">+</b></summary><div className="experience-detail"><p>{item.description}</p><div className="experience-contributions"><h4>Responsibilities and contributions</h4><ul>{item.highlights.map(point => <li key={point}>{point}</li>)}</ul></div></div></details>)}</div></section>
      <ContactCTA />
    </main>
  );
}
